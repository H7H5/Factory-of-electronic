...
public class Machine_device 
{
    ...
    public void Insert_function(unitySQLite unitySQLite, string data)
    {
        ...
        using (dbconn = new SqliteConnection(unitySQLite.conn))
        {
            ...
            sqlQuery = string.Format("insert into machines_device (data) " + "values (\"{0}\")", "dfdf");
    ...
    public void Update_function(unitySQLite unitySQLite, int id, string data)
    {
        ...
        using (dbconn = new SqliteConnection(unitySQLite.conn))
        {
            dbconn.Open(); 
            dbcmd = dbconn.CreateCommand();
            sqlQuery = string.Format("UPDATE machines_device set data = @data where id = @id ");
    ...
    public void Reader_functionAllid(unitySQLite unitySQLite, GameObject MachinParent)
    {
        ...
        using (dbconn = new SqliteConnection(unitySQLite.conn))
        {
            ...
            string sqlQuery = "SELECT id " + "FROM machines_device";
            dbcmd.CommandText = sqlQuery;
    ...
    public void Delete_function(unitySQLite unitySQLite, int id)
    {
        ...
        if (id != 0)
        {
            using (dbconn = new SqliteConnection(unitySQLite.conn))
            {
                dbconn.Open(); 
                IDbCommand dbcmd = dbconn.CreateCommand();
                string sqlQuery = "DELETE FROM machines_device where id =" + id;
                dbcmd.CommandText = sqlQuery;
                IDataReader reader = dbcmd.ExecuteReader();
                dbcmd.Dispose();
                dbcmd = null;
    ...
    public void Reader_function(unitySQLite unitySQLite, GameObject MachinParent)
    {
        ...
        using (dbconn = new SqliteConnection(unitySQLite.conn))
        {
            dbconn.Open(); 
            IDbCommand dbcmd = dbconn.CreateCommand();
            string sqlQuery = "SELECT data " + "FROM machines_device";
            dbcmd.CommandText = sqlQuery;
            IDataReader reader = dbcmd.ExecuteReader();
            while (reader.Read())
            {
                data = reader.GetString(0);
                MachinParent.GetComponent<MachineParentScript>().CreateOneMachineDevice(data);
            }
            reader.Close();
            reader = null;
            dbcmd.Dispose();
